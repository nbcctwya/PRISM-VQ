...
from trainer.train_ypred_autoregressive import GenerateReturn
...
def _ensure_config_snapshot() -> Path:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix='stage2_ar_', dir=str(snapshot_root)))
...
def _compose_frozen_config(overrides: List[str]) -> DictConfig:
    ...
    with initialize_config_dir(config_dir=str(snapshot_dir), job_name='stage2_ar_frozen'):
...
def _build_run_name(cfg: DictConfig) -> str:
    ...
    return (
        ...
        f"1d{dropout_pred}_1{distance}_p{horizon}_ai{aux_imp}_ks{kernel_size}_AR"
    )

def _build_wandb_logger(cfg: DictConfig, run_name: str) -> WandbLogger:
    project_name = f"{cfg.train.project_name}_{cfg.data.universe}_stage2_ar"
...
def main(cfg: DictConfig) -> float:
    ...
    print(f"********** Architecture: Autoregressive Transformer (GPT-style) **********")
