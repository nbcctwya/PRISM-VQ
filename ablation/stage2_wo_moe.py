...
from trainer.train_ypred_wo_moe import GenerateReturn
...
_SNAPSHOT_ENV_KEY = "STAGE2_WO_MOE_CONFIG_SNAPSHOT_DIR"
...
def _ensure_config_snapshot() -> Path:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix='stage2_wo_moe_', dir=str(snapshot_root)))
...
def _compose_frozen_config(overrides: List[str]) -> DictConfig:
    ...
    with initialize_config_dir(config_dir=str(snapshot_dir), job_name='stage2_wo_moe_frozen'):
...
def _build_run_name(cfg: DictConfig) -> str:
    ...
    return (
        f"{cfg.train.seed}_VQ{num_embed}_{cfg.data.universe}_woMOE_"
        ...
        f"1d{dropout_pred}_1{distance}_p{horizon}_ai{aux_imp}"
    )

def _build_wandb_logger(cfg: DictConfig, run_name: str) -> WandbLogger:
    project_name = f"{cfg.train.project_name}_{cfg.data.universe}_stage2_wo_moe"
...
def train(cfg: DictConfig,
    ...
    run_dir = Path(get_root_dir()) / cfg.train.save_res / 'wo_moe' / save_run_name
