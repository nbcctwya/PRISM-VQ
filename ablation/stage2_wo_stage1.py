...
from trainer.train_ypred_wo_stage1 import GenerateReturn
...
_SNAPSHOT_ENV_KEY = "STAGE2_WO_STAGE1_CONFIG_SNAPSHOT_DIR"
...
def _ensure_config_snapshot() -> Path:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix='stage2_wo_stage1_', dir=str(snapshot_root)))
...
def _compose_frozen_config(overrides: List[str]) -> DictConfig:
    ...
    with initialize_config_dir(config_dir=str(snapshot_dir), job_name='stage2_wo_stage1_frozen'):
...
def _build_run_name(cfg: DictConfig) -> str:
    ...
    # No Stage 1 here, so saved_model parsing is skipped.
    return (
        f"{cfg.train.seed}_{cfg.data.universe}_woStage1_"
        f"mo{n_expert}_k{k}_mh{moe_hidden}_md{moe_drop}_dm{dim}_nh{n_heads}_l{n_layer}_"
        f"d{dropout}_au{aux_weight}_p{horizon}_ai{aux_imp}_ks{kernel_size}"
    )

def _build_wandb_logger(cfg: DictConfig, run_name: str) -> WandbLogger:
    project_name = f"{cfg.train.project_name}_{cfg.data.universe}_stage2_wo_stage1"
...
def train(cfg: DictConfig,
    ...
    model = GenerateReturn(config_dict, T_max=T_max)
    ...
    best_model = GenerateReturn.load_from_checkpoint(best_path, config=config_dict, T_max=T_max)
    best_model.eval()

    with torch.no_grad():
        print("========== Validation evaluation ==========")
        _, _, val_metric = run_inference(best_model, valid_loader, config_dict)
        print(f"Validation RIC: {val_metric['RankIC']:.4f}")

        print("========== Test evaluation ==========")
        pred_df, _, metric = run_inference(best_model, test_loader, config_dict)
        print(f"Test RIC: {metric['RankIC']:.4f}")

    save_run_name = '_'.join(run_name.split('_')[1:])
    root_dir = Path(get_root_dir())
    run_dir = root_dir / cfg.train.save_res / 'wo_stage1' / save_run_name

    print(f"========== DEBUG: Root dir = {root_dir} ==========")
    print(f"========== DEBUG: Run dir = {run_dir} ==========")
    print(f"========== DEBUG: Creating directory... ==========")

    run_dir.mkdir(parents=True, exist_ok=True)

    seed = cfg.train.seed
    output_path = run_dir / f"{seed}_best.pkl"
    print(f"========== DEBUG: Saving predictions to {output_path} ==========")
    pred_df.to_pickle(output_path)

    output_csv_path = run_dir / f"{seed}_metric.csv"
    print(f"========== DEBUG: Saving metrics to {output_csv_path} ==========")
...
def main(cfg: DictConfig) -> float:
    ...
    metric = train(frozen_cfg, config_dict, train_loader, valid_loader, num_batches_per_epoch_train, test_loader)
    return metric.get('RankIC', 0.0)
