...
class SparseDispatcher(object):
    ...
    def combine(self, expert_out, multiply_by_gates=True):
        ...
        stitched = torch.cat(non_empty_expert_out, 0)
        if multiply_by_gates:
            # stitched = stitched.mul(self._nonzero_gates)
            stitched = torch.einsum("i...,ij->i...", stitched, self._nonzero_gates)

        shape = list(non_empty_expert_out[-1].shape)
...
class FactorGatedMoE(nn.Module):
    """Call a Sparsely gated mixture of experts layer with simple MLP networks as experts.
    Args:
    ...
    def forward(self, x, z, loss_coef=1): #1e-2):
        ...
        # x_norm = rearrange(x_norm, "x l (y c) -> (x y) l c", y=self.n_vars) # (B, T, 158) -> (B, T, 158)
        # else:
