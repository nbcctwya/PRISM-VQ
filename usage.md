...
`stage2.py` 会根据 `data.universe` 自动应用 `configs/config.yaml` 中的 `stage2_presets`：

```text
csi300: saved_model=infucsi300..., aux_weight=0.01,  n_expert=2, num_heads=2
sp500 : saved_model=infusp500...,  aux_weight=0.001, n_expert=8, num_heads=4
```

其中 `predictor.k` 默认是 `${half:${predictor.n_expert}}`，通常不用手动改。命令行里显式传入的 `predictor.*` 仍然优先，做消融或 sweep 时可以继续覆盖单个参数。
